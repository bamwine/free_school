...
<form action="index.php?admin/grade" method="post" accept-charset="utf-8">
                <table class="table table-bordered datatable" id="table_export">
                    ...
                    <tbody>
                        ...
                        <tr>
							<td><input value="<?php echo $row['name'];?>" type="text" name="name_<?php echo $row['grade_id'];?>" class="form-control" /></td>
							<td><select name="comment_<?php echo $row['grade_id'];?>" class="form-control " style="width:100%;">
	
		<option value=""><?php echo get_phrase('select');?></option>
		<option value="Fail" <?php if($row['comment'] =='Fail'){echo 'selected';}?>>Fail</option>
		<option value="Pass" <?php if($row['comment'] =='Pass'){echo 'selected';}?>>Pass</option>
		<option value="Credit" <?php if($row['comment'] =='Credit'){echo 'selected';}?>>Credit</option>
		<option value="Distinction" <?php if($row['comment'] =='Distinction'){echo 'selected';}?>>Distinction</option>
...
function get_class_sections(class_id) {
    	...
    	$.ajax({
            url: '<?php echo base_url();?>index.php?admin/admin/grade/check' + class_id ,
