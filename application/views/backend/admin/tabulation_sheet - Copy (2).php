...
foreach($subject as $subjectrow):?>
  ...
  <td style="text-align: center;"><?php echo $subjectrow['name'];?></td>
...
foreach($subjects as $subjectrow){
			...
			$this->db->get_where('mark' , array('class_id'=>$class_id,'exam_id'=>$exam_id))->result_array();
				//echo $subjectrow['subject_id'].'<br>';
				foreach($marks as $marksrow){ ?> 
					
				<td style="text-align: center;"><?php echo $marksrow['mark_obtained'];?></td>	
