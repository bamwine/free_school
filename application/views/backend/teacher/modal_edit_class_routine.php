...
<form action="<?php echo base_url();?>index.php?teacher/class_routine/do_update/<?php echo $rowdata['class_routine_id']?>" class="form-horizontal validatable" target="_top" method="post" accept-charset="utf-8">
...
<div class="col-sm-5">
<select name="subject_id" class="form-control selectboxit visible" style="display: none;">
<?php 
	$clas = $this->db->get('subject')->result_array();
	foreach($clas as $row3):
		?>
		<option value="<?php echo $row3['subject_id'];?>"
			<?php if($rowdata['subject_id'] == $row3['subject_id'])echo 'selected';?>>
					<?php echo $row3['name'];?>
				</option>
	<?php
	endforeach;
...
<button type="submit" class="btn btn-info">edit class routine</button>
</div>
</div>
</form>
</div>
