...
//            $diff = date_diff($cur_time, $send_time);
	 ...
	 function expiry_remaining_time ()
    {
		$start_date = date('Y-m-d');
		$queryr = $this->db->query('SELECT * FROM valid');
		$tkl = $queryr->row()->tkl;
        ...
        $since_start = $start_date->diff(new DateTime($expire));
	...
	function get_fess($class_id,$amount) {
...
{			
	$this->db->where('class_id', $class_id);
	$this->db->where('term', $term_name);
	$this->db->where('year', $year);
	$this->db->update('fees_struct', array('amount' => $amount));
	
        
	}else if ($check->num_rows() ==0){
		
		
		 $this->db->insert('fees_struct', array('class_id' => $class_id,'term' => $term_name,'year' => $year,'amount' => $amount));
	...
	function get_all_payments($student_id) {
		
		$term_name = $this->db->get_where('settings' , array('type'=>'term'))->row()->description;
		$year  =$this->db->get_where('settings' , array('type'=>'years'))->row()->description;
		$check = $this->db->query('SELECT sum(amount_paid) as total FROM invoice WHERE student_id = "'.$student_id.'" AND term = "'.$term_name.'" AND years = "'.$year.'"');
	...
	function get_class_bal_paid() {
		
		$term_name = $this->db->get_where('settings' , array('type'=>'term'))->row()->description;
		$year  =$this->db->get_where('settings' , array('type'=>'years'))->row()->description;
		$check = $this->db->query('SELECT sum(invoice.amount_paid)as paid,sum(invoice.due)as bala,class.name
FROM  invoice INNER JOIN student ON invoice.student_id = student.student_id
INNER JOIN class ON student.class_id = class.class_id  WHERE   invoice.term = "'.$term_name.'" AND invoice.years = "'.$year.'" GROUP BY  class.class_id');
		$res = $check->result_array();
		...
		function get_class_amount($student_id) {
		
		$term_name = $this->db->get_where('settings' , array('type'=>'term'))->row()->description;
		$year  =$this->db->get_where('settings' , array('type'=>'years'))->row()->description;
		$check = $this->db->query('SELECT fees_struct.amount 
FROM  student  INNER JOIN class ON student.class_id = class.class_id
  INNER JOIN fees_struct ON fees_struct.class_id = class.class_id
   WHERE student_id = "'.$student_id.'" AND term = "'.$term_name.'" AND year = "'.$year.'"');
		$res = $check->result_array();
		...
		function get_student_total_paid($student_id) {
		
		$term_name = $this->db->get_where('settings' , array('type'=>'term'))->row()->description;
		$year  =$this->db->get_where('settings' , array('type'=>'years'))->row()->description;
		$check = $this->db->query('SELECT  Sum(invoice.amount_paid) AS total
FROM  invoice WHERE student_id = "'.$student_id.'" AND term = "'.$term_name.'" AND years = "'.$year.'"');
		$res = $check->result_array();
	...
	function get_student_baltopay($student_id,$newpay) {
		//if($this->get_student_total_paid($student_id)> $this->get_class_amount($student_id))
			//$balance; else {
		$balance = ($newpay+ $this->get_student_total_paid($student_id))- $this->get_class_amount($student_id);
			//}
		return $balance;
	}
}
